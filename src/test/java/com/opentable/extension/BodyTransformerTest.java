...
public class BodyTransformerTest {
    ...
    @Test
    public void willReplaceValueCharactersWithGivenOne() {
        wireMockRule.stubFor(get(urlEqualTo("/test?date=2018-06-25"))
            .willReturn(aResponse()
                .withStatus(200)
                .withHeader("content-type", "application/json")
                .withBody("{\"date\": \"$(date|replace('-','')\"}")
                .withTransformers("body-transformer")));

        given()
            .contentType("application/json")
            .when()
            .get("/test?date=2018-06-25")
            .then()
            .statusCode(200)
            .body("date", equalTo("20180625"));

        wireMockRule.verify(getRequestedFor(urlEqualTo("/test?date=2018-06-25")));
    }
